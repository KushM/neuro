    ...
    Pna=gna.*(m.^3).*(hp).*(V-Ena).*(V-Ena);
    Pk=gk.*(n.^4).*(V-Ek).*(V-Ek);
    Pl=gl.*(V-El).*(V-El);
    Pm=((-1*gna.*(m.^3).*(hp).*(V-Ena))+(-1*gk.*(n.^4).*(V-Ek))+(-1*gl.*(V-El))+I).*(V);
    
  
    Energyna=sum(Pna)*h*10^-6
    Energyk=sum(Pk)*h*10^-6
    Energyl=sum(Pl)*h*10^-6
    Energym=sum(Pm)*h*10^-6
